...
const inputcell = ClientFunction(() => 
  document.querySelector('div.ib_container > div.ib_body > div > div.editor').textContent=document.querySelector('div.ib_container > div.ib_body > div > div.editor').textContent+'*1'
...
async clickOnSpecificArea(sourceElement){
  ...
  async gettime()
  {
    ...
    async capturescreenshot(testName,input){
      ...
      await t
      //.switchToMainWindow()
      .takeElementScreenshot(Selector('.matrix-container.ibcs_Tablecontainer'), actualScreenshotAbsolutePath)
      //.takeElementScreenshot(Selector('.viz-container.plot-area'), actualScreenshotAbsolutePath)
    ...
    async doVisualRegression(testName, input) {
      ...
      const { t } = this.helpers.TestCafe;
          ...
          if(fs.existsSync(basepath)===false)
          {
            ...
            await t
            //.switchToMainWindow()
            //.takeElementScreenshot(Selector('.viz-container.plot-area'),baseScreenshotAbsolutePath)
            .takeElementScreenshot(Selector('.matrix-container.ibcs_Tablecontainer'),baseScreenshotAbsolutePath)
          ...
          //Image Comparison
           ...
           {
             console.log('Base Path of the file::'+basepath)
            if(fs.existsSync(actpath)===true)
            {    
              console.log('Actual Path of the file::'+actpath)
            await resemble(basepath)
              ...
              .onComplete(async data => {
                ...
                console.log('Image difference Percentage:'+imageDifferentPercentage)
      ...
      //.click('div.upload-text');
       }

       async fileuploadFooter()
       {
        const { t } = this.helpers.TestCafe;
        await t
        .setFilesToUpload('div.footerEditor #imageFile', [
          './visualbi.jpg'
      ])
      //.click('div.upload-text');
       }

      async draganddrop(loc1)
       {
        const { t } = this.helpers.TestCafe;
        const loc = Selector(loc1);
        await t
       // .hover(loc)
       // .click(loc)
        //.dragToElement(loc1,loc2)
        .drag(loc, 200, 200)
        .drag(loc, 100, 100)
        .drag(loc, 100, 400)
        .drag(loc, 100, 200)
        .drag(loc, 100, 300)
        //.drag(loc, 200, 200, { offsetX: 10, offsetY: 10 })
      //.click('div.upload-text');
       }
      
       async refreshthePage(current_url)
       {
        const { t } = this.helpers.TestCafe;
